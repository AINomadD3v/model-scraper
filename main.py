...
class AirtableClient:
    ...
    def get_active_accounts(
        self,
    ) -> List[Tuple[str, str, int]]:  # Updated return type to include followers
        ...
        try:
            ...
            for record in records:
                ...
                self.content_table.update(
                    record["id"], {"Previous Views": current_views}
                )
    ...
    def upsert_content(self, content_data: Dict[str, Any]) -> bool:
        ...
        try:
            ...
            if existing_records:
                ...
                self.logger.info(
                    f"Created view history record for content: {content_id}"
                )
            else:
                ...
                self.view_history.create(history_record)
                self.logger.info(
                    f"Created initial view history record for content: {content_id}"
                )
    ...
    def _format_content_data(self, post_data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        return {
            ...
            "Account": (
                [post_data.get("account_id")] if post_data.get("account_id") else None
            ),
            ...
            "Thumbnail": (
                [{"url": post_data.get("thumbnail_url")}]
